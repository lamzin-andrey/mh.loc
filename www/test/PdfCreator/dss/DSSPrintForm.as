package drake.dss{
	...
	public class DSSPrintForm extends Sprite {
		private var LIST_PER_PRINT = 39; //Количество листов стоимостью "одна печать", может быть переписано с помощью flashVars
		private var MAX_RECOVERY_HELP_LINK = "/help.php"; //Ссылка "Как это исправить см в разделе помощь" при количестве листов более чем лимит, может быть переписано с помощью flashVars
		private var dssLoader:DSSLoader;
		//private var pj:PrintJob;
		private var formView:PrintForm;
		private var formViewShadow:Sprite;
		private var viewW:Number;
		private var viewH:Number;
		private var viewX:Number;
		private var viewY:Number;
		private var responder:CResponder;
		private var dlg:DSSAlertDialog;
		private var printInfo:Array;
		private var fileName :String;
		private var confirmDlg :DSSConfirmPrintDialog;
		private var confirmDialogShadow:Sprite;
		public  var fileID:String;
		//public  var accountShadow:DSSAccountShadow;
		private var handCursor:HandView;
		...
		*/
		...
		private function onPrintBtnClick(e:MouseEvent)
		{
				...
				if (Number(printInfo["balance"]) > 0 && Number(printInfo["oneListPrice"]) > 0) {
					...
					var price = 0;
					while (n > 0) { n -= LIST_PER_PRINT; price += onePrice; }
		...
		*/
		private function _setPrintInfoViewOnFreePrint(quantity) {
			...
			if (quantityA4 > LIST_PER_PRINT) {
				setText(formView.mc_maximum_text_info.txt_limit_one_print, LIST_PER_PRINT + ' ' + Utils.plural(LIST_PER_PRINT, ['лист', 'листа', 'листов'] ) );
		...
		*/
		private function _setPrintInfoView(quantity) {
			...
			while (n > 0) { n -= LIST_PER_PRINT; price += oneListPrice; }
		...
		*/
		private function _setMaxRecoveryLink() {
			...
			s = s ? s : MAX_RECOVERY_HELP_LINK;
			MAX_RECOVERY_HELP_LINK = s;
		...
		*/
		private function onRecoveryMaxClick(evt:MouseEvent) {
			navigateToURL( new URLRequest(MAX_RECOVERY_HELP_LINK), "_blank" );
		}
		/**
		 * @desc console.log
		*/
