<?php
/** Класс для работы с postgtresql */
...
class PgDB {
	...
	static public function setConnection(&$error, $host = '', $user = '', $password = '', $dbname = '', $port = '') {
		...
		try {
			//TODO set port
			$db = new PDO('pgsql:dbname=' . $dbname . ';host=' . $host . ';port=' . $port, $user, $password);
			return $db;
	 ...
	 * @param array  $arguments содкржит значения плейсхолдеров, в случае insert запроса 0 элемент содержит имя последовательности
	 * @param int    &$numRows
	 * @return array || int last insert record id
	*/
	static public function query($cmd, $arguments = array(), &$numRows = 0, &$affectedRows = 0) {
	...
	*/
	static public function row($cmd, $arguments = array(), &$numRows = 0) {
		$rows = self::query($cmd, $arguments, $numRows);
		if ($numRows) {
			return $rows[0];
		}
		return false;
	 ...
	 * @return array || bool false
	*/
	static public function val($cmd, $arguments = array(), &$numRows = 0) {
		$row = self::rows($cmd, $arguments, $numRows);
